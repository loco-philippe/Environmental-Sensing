...
def post():
    ...
    print(content)
    try:
        if "request_type" in content and (
            content["request_type"] == "mongo-validation"
            or content["request_type"] == "mongo-validation-namelist"
        ):
            try:
                client = MongoClient([content["uri"]])
                collection = client[content["database_name"]][
            ...
            print(srch.request)
            result = srch.execute(returnmode="single").to_obj(
                encoded=True, modecodec="dict", geojson=True
            )
            print(result)
        ...
        )  # ne fonctionne pas, 499 n'est pas un code d'erreur accepté par abort
    return ""
