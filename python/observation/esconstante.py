...
def _classval():
    ...
    return {
        ...
        ES.ili_clsName: Dataset,
        ES.iin_clsName: Field,
...
class Es:
    ...
    def _inv(self, mp):
        return {val: key for key, val in mp.items()}

    # def _inv(self, mp)     : return dict(zip(mp.values(), mp.keys()))

    def _invnum(self, mp):
        return dict(zip([k[0] for k in list(mp.values())], mp.keys()))
    ...
    def _initName(self):
        ...
        self.ili_clsName = "Dataset"
        self.iin_clsName = "Field"
