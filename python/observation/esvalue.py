...
class DatationValue(ESValue):  # !!! début ESValue
    ...
    *getters*
    ...
    def boundingBox(listValue):
        ...
        return [
            min([val.value.slot[0].start for val in listValue]),
...
class LocationValue(ESValue):  # !!! début LocationValue
    ...
    *getters*
    ...
    def __lt__(self, other):
        ...
        return distance.distance(self.coorInv, ES.distRef) < distance.distance(
            other.coorInv, ES.distRef
        )
    ...
    def boundingBox(listValue):
        ...
        return [
            min([val.value.bounds[0] for val in listValue]),
    ...
    def vSimple(self, string=False):
        ...
        if string:
            return json.dumps(
                [round(self.value.centroid.x, 5), round(self.value.centroid.y, 5)],
                cls=ESValueEncoder,
            )
    ...
    def vPointInv(self, string=False):
        ...
        if string:
            return json.dumps(
                [self.vSimple()[1], self.vSimple()[0]], cls=ESValueEncoder
            )
        return [self.vSimple()[1], self.vSimple()[0]]
        ...
        for tpe in ["Point", "MultiPoint", "Polygon", "MultiPolygon"]:
            try:
                return shapely.geometry.shape(
                    geojson.loads('{"type":"' + tpe + '","coordinates":' + coor + "}")
                )
            except:
                pass
    ...
    def __init__(self, val=ES.nullPrp, name=ES.nullName, prp_dict=False):
        ...
        *Parameters*
        ...
        if isinstance(val, self.__class__):
            ...
            self.value = val.value
            return
        if val is not None and name == ES.prp_type:
            name = None
            val = {ES.prp_type: val}
...
class NamedValue(ESValue):  # !!! début ResValue
    ...
    *getters*
    ...
    def _jsonValue(self, **option):
        ...
        if isinstance(
            self.value,
            (int, str, float, bool, list, dict, datetime.datetime, type(None), bytes),
        ):
            return self.value
...
class ExternValue(ESValue):  # !!! début ResValue
    ...
    *getters*
    ...
    def _jsonValue(self, **option):
        ...
        if self.value.__class__.__name__ in ["Field", "Dataset", "Observation"]:
