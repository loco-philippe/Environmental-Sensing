...
dico_alias_mongo = {  # dictionnary of the different names accepted for each comparator and each given type. <key>:<value> -> <accepted name>:<name in MongoDB>
    ...
    list: {  # lists are interpreted as geometries
        ...
        "geoNear": "$geoNear",
        "in": "$in",
        "$in": "$in",  # only in case where a list is not a geometry
    },
...
dico_alias_python = {
    ...
    "geometry": {  # lists are interpreted as geometries
        ...
        "disjoint": _geodis,
        "$disjoint": _geodis,
        "intersects": _geointer,
        "$intersects": _geointer,
        "geoIntersects": _geointer,
        "$geointersects": _geointer,
        "geoIntersects": _geointer,
        "$geoIntersects": _geointer,
        "touches": _geotou,
...
def insert_from_doc(collection, document, info=True):
    ...
    with open(document, "r") as doc:
        for line in doc:
            try:
                insert_to_mongo(collection, line, info)
            except:
                pass
...
def insert_to_mongo(collection, obj, info=False):  # Mieux avec panda ?
    ...
    for obs in pile:
        ...
        if obs.name:
            metadata["name"] = obs.name
        if obs.param:
            metadata["param"] = obs.param
        if info:
            metadata["information"] = Observation._info(True, True)
        if (
            len(obs.lname) == 1
        ):  # a special case is needed because lists with one element are replaced by the element itself so iteration doesn't work
            for line in obs:
                inserted_list.append(
                    {
                        obs.lname[0]: util.json(
                            line,
                            encoded=False,
                            typevalue=None,
                            simpleval=False,
    ...
    if inserted_list != []:
        collection.insert_many(inserted_list)
...
class ESSearch:
    ...
    *query method*
    ...
    def __init__(
        self, input=None, parameters=None, hide=[], heavy=False, sources=None, **kwargs
    ):
        """
        ...
        *Arguments*
        ...
        if isinstance(hide, list):
            self.hide = hide  # self.hide
        else:
            raise TypeError("hide must be a list.")

        if isinstance(heavy, bool):
            self.heavy = heavy  # self.heavy
        else:
            raise TypeError("heavy must be a bool.")
        self.sources = sources  # self.sources

        self.input = [
            [],
            [],
        ]  # self.input : formatted as [[Mongo Objects], [Observations]] (list of two lists)
        ...
        while not len(pile) == 0:
            ...
            if isinstance(obj, list):
                ...
                try:
                    ...
                    raise ValueError(
                        "Cannot convert " + str(obj) + " to an Observation."
                    )
            elif obj is not None:
                raise TypeError("Unsupported type for input " + str(obj))

        if parameters:
            self.addConditions(parameters)
        if kwargs:
    ...
    def addInput(self, input):
        ...
        while (
            not len(pile) == 0
        ):  # using a stack (LIFO) to allow easy treatment of nested data.
            obj = pile.pop()
            if isinstance(obj, list):
                pile += obj
            elif isinstance(obj, (Collection, Cursor, CommandCursor)):
    ...
    def addConditions(self, parameters):
        ...
        if isinstance(parameters, dict):  # case when one single condition is added
        ...
        ):  # case when several conditions are added
            ...
            raise TypeError("parameters must be either a dict or a list of dict.")

    def addCondition(
        self, path, operand=None, comparator=None, or_position=-1, **kwargs
    ):
        """
        ...
        *Parameters*
        ...
        - **path** :  str (required argument) - name of an IIndex, which corresponds to an Dataset column name, or name of a metadata element.
        ...
        if comparator is not None and not isinstance(comparator, str):
            raise TypeError("comparator must be a str.")
        if or_position is not None and not isinstance(or_position, int):
            raise TypeError("or_position must be an int.")
        ...
        if isinstance(operand, datetime.datetime) and (
            operand.tzinfo is None or operand.tzinfo.utcoffset(operand) is None
        ):
            operand = operand.replace(tzinfo=datetime.timezone.utc)

        if operand:  # checking if comparator can be applied on the operand
            try:
                comparator = dico_alias_mongo[type(operand)][comparator]
            except:
                raise ValueError(
                    "Incompatible values for comparator and operand. Ensure parameters are in the correct order."
                )
        elif comparator:
    ...
    def removeCondition(self, or_position=None, condnum=None):
        ...
        if or_position is None:
            if condnum is None:  # by default : remove the very last added condition.
                if len(self.parameters[-1]) > 1:
                    self.parameters[-1].pop(-1)
                else:
                    self.parameters.pop(-1)
            else:
                if len(self.parameters[-1]) > 1 or condnum > 1:
                    self.parameters[-1].pop(condnum)
                else:
                    self.parameters.pop(-1)
        else:
            if condnum is None or (
                len(self.parameters[or_position]) == 1 and condnum == 0
            ):
                self.parameters.pop(
                    or_position
                )  # if or_position is not None and condnum is, the whole sublist at or_position is removed.
            else:
                self.parameters[or_position].pop(condnum)
        if (
            self.parameters == []
        ):  # ensure self.parameters returns to its default value after being emptied
            self.parameters = [[]]
        ...
        **kwargs,
        ...
        if unwind:
            if isinstance(unwind, str):
                ...
                for _ in range(unwind[1]):
                    self._unwind.append(unwind[0])
            else:
                raise TypeError("unwind must be a tuple, a str or an int.")

        if self.heavy and operand is not None:
            if path not in self._heavystages:
                self._heavystages.add(
                    path
                )  # peut-être mieux de laisser l'utilisateur choisir manuellement
            path = "_" + path + ".v"

        if (
            operand is None
        ):  # no operand => we only test if there is something located at path
            comparator = "$exists"
            operand = 1
        else:
            try:
                comparator = dico_alias_mongo[type(operand)][
                    comparator
                ]  # global variable
            except:
                if formatstring:
                    try:
                        comparator = dico_alias_mongo[datetime.datetime][comparator]
                    except:
                        raise ValueError("Comparator not allowed.")
                elif isinstance(operand, shapely.geometry.base.BaseGeometry):
                    operand = {
                        "type": operand.geom_type,
                        "coordinates": list(operand.exterior.coords),
                    }
                else:
                    raise ValueError("Comparator not allowed.")
        ...
        if isinstance(
            operand, TimeSlot
        ):  # equals->within, contains->intersects, within, disjoint, intersects
            self._filtered = True
            if comparator == "within":
                ...
                self._cond(
                    or_pos, operand[0].start, "$lte", path, False
                )  # pourquoi False et pas inverted ici ??
        ...
        if comparator in {"$geoIntersects", "$geoWithin"}:  # operand :
            ...
            if isinstance(operand, list):
                if not isinstance(operand[0], list):
                    ...
                    if len(operand) == 1:
                        ...
                        raise ValueError(
                            "Unable to define a geometry from " + str(operand)
                        )
                else:
                    geom_type = "Polygon"
                    coordinates = operand
            ...
            if "distanceField" not in self._geonear:
                raise ValueError("distanceField missing in MongoDB stage $geoNear.")
            return
        elif isinstance(
            operand, list
        ):  # lists are interpreted as geometries. An additional filtering is necessary for geometry-specific functions
            self._filtered = True
            return
        ...
        if inverted:
            if path in self._match[or_pos]:
                if "$nor" in self._match[or_pos][path]:
                    ...
                    self._match[or_pos][path]["$nor"] = [
                        self._match[or_pos][path]["$not"],
                        cond_0,
                    ]
    ...
    def _fullSearchMongo(self):
        ...
        if (
            not self._unwind and not self.heavy and not self._set and not self._geonear
        ...
        ## ESSearch._fullSearchMongo() 4: Case 2 : aggregate request
            ...
            if self._heavystages:  # Additional Mongo $set stage if self.heavy is True
                ...
                for path in self._heavystages:
                    heavy |= {
                        "_" + path: {
                            "$cond": {
                                "if": {"$eq": [{"$type": "$" + path}, "object"]},
                                "then": {"$objectToArray": "$" + path},
                                "else": {"v": "$" + path},
                            }
                        }
                    }
            ...
            if self._set:
                request.append({"$set": self._set})  # Mongo $set stage
            if self._geonear:
                request.append({"$geoNear": self._geonear})  # Mongo $geoNear stage
    ...
    def cursor(self):
        ...
        for item in self.input[
            0
        ]:  # Determine the result cursor for each element of the input on which a Mongo query makes sense
    ...
    def execute(self, returnmode="observation", fillvalue=None, name=None, param=None):
        ...
        *Parameter*
        ...
        """
        if returnmode not in {"unchanged", "observation", "idfused", "single"}:
            raise ValueError(
                "returnmode must have one of these values: 'unchanged', 'observation', 'idfused', 'single'."
            )
        if returnmode == "single":
            if name is not None and not isinstance(name, str):
                raise TypeError("name should be a string.")
            if param is not None and not isinstance(param, dict):
                raise TypeError("param should be a dictionnary.")
        ...
        for data in self.input[0]:
            ...
            if (
                returnmode == "observation"
            ):  # Only in this case is an observation created directly.
                for item in cursor:
                    if (
                        self._filtered
                    ):  # Additional filtering for objects like TimeSlot who need it
                            ...
                            for cond in conds:
                                if cond["path"] in item:
                                    try:
                                        ...
                                        )  # checking for each condition if it is satisfied
                                    except:
                                        checks_conds = False
                                else:
                                    checks_conds = False
                    ...
                    if "_metadata" in item:
                        if "name" in item["_metadata"]:
                            dic["name"] = item["_metadata"]["name"]
                        if "param" in item["_metadata"]:
                            dic["param"] = item["_metadata"]["param"]
                        del item["_metadata"]
                    dic["idxdic"] = {key: [item[key]] for key in item}
                    if not self._filtered or (self._filtered and checks_parameters):
        ...
        if returnmode == "single":
            ...
            for i in range(len(result)):
                ...
                for column_name in result[
                    i
                ]:  # for columns missing in the new Observation
                    if column_name not in arg:
                        arg[column_name] = (
                            [fillvalue] * i + [result[i][column_name]]
                        )  # an empty column filled with fillvalue is added if a new column name is encountered
                    else:
                        arg[column_name].append(result[i][column_name])
            if self._filtered:
                result = [self._filtered_observation(Observation.dic(arg))]
            else:
                result = [Observation.dic(arg)]
            ...
            for item in result:
                id = str(
                    item["_metadata"]["id"]
                )  # will throw an error if item has no id. Should items with no id be let as is or merged together?
                if (
                    id in hashs_dic
                ):  # one line is added to hashs_dic[id] for each element of result having this id
                    del item[
                        "_metadata"
                    ]  # Two items with the same id should have the same metadata.
                    ...
                    for column_name in item:
                        if column_name not in hashs_dic[id]["idxdic"]:
                            hashs_dic[id]["idxdic"][column_name] = (
                                [fillvalue] * i + [item[column_name]]
                            )  # a filled column is added if a new column name is encountered
                        else:
                            hashs_dic[id]["idxdic"][column_name].append(
                                item[column_name]
                            )
                else:
                    dic = {}
            ...
            for id in hashs_dic:  # an Observation is added to result for each id
                ...
                if obs_out:
                    if self._filtered:
                        result.append(
                            self._filtered_observation(obs_out)
                        )  # finalement, semble plus pertinent de faire ce filtrage directemt sur la sortie Mongo, car même si un à un les tests de condition sont faits à de nombreuses reprises, au global ce ne sont jamais les mêmes combinaisons de test
                    else:
        ...
        for data in self.input[
            1
        ]:  # data which are not taken from a Mongo database and already are observations are treated here.
        ...
        if len(result) >= 1:
            if returnmode == "single":
                ...
                if name is None:
                    name = "ESSearch query result on " + str(
                ...
                if param is None:  # default value for param
                    if self.sources is not None:
                        ...
                        for item in (
                            self.input
                        ):  # informations about the inputs are added to param
                ...
                for id in hashs_dic:
                    ...
                    if hashs_dic[id][0].name:
                        obs_out.name = hashs_dic[id][
                            0
                        ].name  # name and param associated to the id are put back
                    if hashs_dic[id][0].param:
                        obs_out.name = hashs_dic[id][0].param
                    if obs_out:
                        result.append(obs_out)
    ...
    def _filtered_observation(
        self, obs, is_from_mongo=True
    ):  # Vérifier que cette fonction n'est pas moins efficace que de tout déplier / filtrer / tout replier
        ...
        if len(obs) == 0 or self.parameters == [[]]:
            return obs
        ...
        for i in range(
            len(self.parameters)
        ):  # for each group of conditions separated by a or
            if self.parameters[i] != []:
                ...
                for cond in self.parameters[i]:
                    ...
                    for j in range(len(obs.lindex)):
                        if cond["path"] == obs.lindex[j].name:
                            ...
                            cond["path"][:9] == "_metadata"
                        ):  # metadata are id, name and param
                            if not is_from_mongo:  # This step is considered to be done already for data taken out of Mongo
                                if cond["path"][10:] == "name":
                                    next_relevant = next_relevant and self._condcheck(
                                        obs.name, cond
                                    )
                                if cond["path"][10:] == "id":
                                    next_relevant = next_relevant and self._condcheck(
                ...
                if not relevant:
                    continue  # if a column on which a condition is applied is missing, the set of conditions given by the element of self.parameters is considered not verified.
                ...
                for j in range(obs.lenidx):  # iteration over the columns
                    ...
                    for item in obs.lindex[j].cod:
                        ...
                        for cond in conds[j]:
                            try:
                                boolean = boolean and self._condcheck(item, cond)
                            except:
                                pass  # boolean = False # pose problème pour les regex et autres opérations non implémentées...
                        filter.append(
                            boolean
                        )  # condition is rewritten as a boolean filter for the incoming data
                    next_full_filter = util.tovalues(
                        obs.lindex[j].keys, filter
                    )  # filter changed from filter on optimize codec to filter on full codec. (filter on optimize codec was the just how we calculated it, this isn't an actual method of the module)
                    full_filter = [
                        full_filter[k] and next_full_filter[k]
                        for k in range(len(full_filter))
                    ]  # full filter is updated each time

                ## ESSearch._filtered_observation() 3: or_position is taken into account

                final_filter = [
                    final_filter[j] or full_filter[j] for j in range(len(full_filter))
                ]
    ...
    def _condcheck(self, item, cond=None):  # ajouter la gestion des regex
        ...
        if "formatstring" in cond:
            ...
            if not isinstance(cond["operand"], (datetime.datetime, TimeSlot)):
                cond["operand"] = datetime.datetime.strptime(
                    cond["operand"], cond["formatstring"]
                )
        ...
        if isinstance(item, TimeSlot):
            if isinstance(
                cond["operand"], TimeSlot
            ):  # if comparator is one of the specific operators for TimeSlot
                cond["comparator"] = dico_alias_python[TimeSlot][TimeSlot][
                    cond["comparator"]
                ]
                return item.link(cond["operand"])[0] == cond["comparator"]
            else:  # if operand is a datetime
                try:
                    return dico_alias_python[TimeSlot][datetime.datetime][
                        cond["comparator"]
                    ](item, cond["operand"])
                except:
                    raise ValueError("Comparator not supported for TimeSlot.")

        elif isinstance(item, list) or isinstance(
            item, shapely.geometry.base.BaseGeometry
        ):
            if isinstance(item, list):  # lists are interpreted as geometries
                if len(item) == 1:
                    item = shapely.geometry.Point(item[0])
                elif len(item) > 1 and not isinstance(item[0], list):
                    item = shapely.geometry.Point(item)
                elif len(item) == 2:
                    item = shapely.geometry.LineString(item)
                elif len(item) > 2:
            ...
            if isinstance(cond["operand"], list):  # lists are interpreted as geometries
                if len(cond["operand"]) == 1:
                    cond["operand"] = shapely.geometry.Point(cond["operand"][0])
                elif len(cond["operand"]) > 1 and not isinstance(
                    cond["operand"][0], list
                ):
                    cond["operand"] = shapely.geometry.Point(cond["operand"])
                elif len(cond["operand"]) == 2:
                    cond["operand"] = shapely.geometry.LineString(cond["operand"])
                elif len(cond["operand"]) > 2:
                    if not item[-1] == item[0]:
                        item.append(item[0])
                    item = shapely.geometry.Polygon([item])
            return dico_alias_mongo["geometry"][cond["comparator"]](
                item, cond["operand"]
            )

        elif (
            cond["path"] == "property" and isinstance(item, dict)
        ):  # assuming that property contains dicts and that the query targets one of its values
        ...
        try:
            return dico_alias_mongo["default"][cond["comparator"]](
                item, cond["operand"]
            )
        except:
    ...
    def _fusion(
        self, obsList, fillvalue=None
    ):  # Idéalement, utiliser une méthode fusion de Observation.
        """
        ...
        if len(obsList) == 0:
            ...
            for i in range(len(new_lname)):  # for each column of the new Observation
                ...
                for obs in obsList:  # for each Observation in the list
                    if new_lname[i] in obs.lname:
                        values += obs.lindex[
                            obs.lname.index(new_lname[i])
                        ].values  # values of the column are added to the new column
                    else:
                        values += [fillvalue] * len(
                            obs
                        )  # when there is no value, filled with fillvalue
                codec = util.tocodec(values)
                lidx.append(Field(codec, new_lname[i], util.tokeys(values, codec)))

            ## ESSearch._fusion() 3: Build the actual Observation

            return Observation(lidx)
            # Il y aurait peut-être moyen d'optimiser un peu en remplaçant Field, util.tokeys et l'appel final d'Observation par des manipulations directes sur les objets.
