...
EsObs: list = [ESSetDatation, ESSetLocation, ESSetProperty, ESSetResult]
'''ordered list for classES '''  

EsValue: list = [DatationValue, LocationValue, PropertyValue]
'''ordered list for ESValue '''  

EsObsDict: dict = {ES.dat_classES : ESSetDatation, 
...
class Observation(ESObject):
    ...
    def _initESObs(self, *args, **kwargs) :
        for arg in args :
            if type(arg) == str :       # creation à partir d'un json "key : [liste]"
                ...
                if len(arg) == 4 : 
                    for i in range(4) :
                        if self.element(ES.esObsClass[i]) == None : EsObs[i](arg[i], self)
            elif type(arg) == tuple : self.append(arg[0], arg[1], arg[2], arg[3])
        for k in kwargs :
            if k in ES.esObsClass and self.element(k) == None and type(kwargs[k]) in EsObs: 
                self.addComposant(kwargs[k])
            elif k in ES.esObsClass and self.element(k) == None : 
                EsObsDict[k](kwargs[k], self)
    ...
    def append(self, *arg) :
        ...
        for i in range(3) :
            ...
            else : ind[i] = self.addValue(EsValue[i](arg[i]))
        self.addValueObservation(arg[3], ind[0], ind[1], ind[2])
        self.majType()
    ...
    def addESObs(self, js, classES = None) :
        ...
        if classES in (None, ES.dat_classES) and isESObs(ES.dat_classES, js) \
            and self.element(ES.dat_classES) == None: ESSetDatation(pObs=self, jObj=js)
        if classES in (None, ES.loc_classES) and isESObs(ES.loc_classES, js) \
            and self.element(ES.loc_classES) == None: ESSetLocation(pObs=self, jObj=js)
        if classES in (None, ES.prp_classES) and isESObs(ES.prp_classES, js) \
            and self.element(ES.prp_classES) == None: ESSetProperty(pObs=self, jObj=js)
        if classES in (None, ES.res_classES) and isESObs(ES.res_classES, js) \
            and self.element(ES.res_classES) == None: ESSetResult  (pObs=self, jObj=js)
    ...
    def addResultValue(self, esValue):
        ...
        '''
        return self.element(ES.res_valueType).addValue(esValue)
    ...
    def full(self, maj=True, allAxes=True) : 
        ...
        allAxes : boolean, optionnal 
            If True, axes are completed with empty ES.ESObs 
        Returns
        -------
        empty list if maj is True, else `ES.ESObs.ESSetResult` value.List
        '''
        if allAxes :
    ...
    def _xlist(self, squeeze=True, fullAxes=False, func=ES._identity, maxname=0):
        ...
        if self.setDatation != None: 
            ...
            #xList['datstr'] = self.setDatation.to_numpy(func = DatationValue.vInstant)
    ...
    def to_xarray(self, dataArray=True, sort=True, complet=False, name=True,
        ...
        fullAxes : Boolean, optional
            Include all the axes if True. The default is False.
        Returns
        -------
    ...
    def plot(self, switch = False, line = True, sort = True, size = 5, marker='o'):
        ...
        marker : Character, optional
            Synbol for each point. The default is marker='o'.
        Returns
        -------
        None.
        '''
        ...
        if len(obx.dims) == 1:
                ...
                #plt.legend(obx.coords['prpstr'].to_index().to_list())
        elif len(obx.dims) == 2:
            order = [0,1]
            ext = ['', '']
            for i in (0,1):
