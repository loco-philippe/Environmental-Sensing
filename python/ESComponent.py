...
from ESconstante import ES, mDistRef
...
class DatationValue(ESValue):
    ...
    def vInstant(self, option = ES.mOption):  
        val = ES.nullDate
        if self.slot != [ES.nullDate, ES.nullDate] : val = [self.slot[0].isoformat(), self.slot[1].isoformat()]
        else: val =  self.instant.isoformat()
        if option["json_dat_instant"]:
            if self.instant != ES.nullDate : val =  self.instant.isoformat()
            elif self.slot != [ES.nullDate, ES.nullDate] : 
                val =  (self.slot[0] + (self.slot[1] - self.slot[0]) / 2).isoformat()
        return val

    def json(self, option = ES.mOption): 
        val = self.vInstant(option)
