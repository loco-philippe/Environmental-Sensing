...
"""        self.assertTrue(Dataset().to_obj() == [])
        self.assertTrue(Dataset.obj([1, 2, 3]).to_obj() == [[1], [2], [3]])
        self.assertTrue(Dataset.obj([[1, 2, 3]]).to_obj() == [[1, 2, 3]])
        self.assertTrue(Dataset.obj(['er', 'er', 'er']).to_obj()
                        == [['er'], ['er'], ['er']])
        self.assertTrue(Dataset.from_obj([1, 2, 3]).to_obj() == [[1], [2], [3]])
    ...
    def test_creation_simple(self):
        ...
        self.assertEqual(Dataset(Dataset.obj ([[0, 2, 0, 2, 0], [10, 0, 20, 20, 15]])),
                         Dataset.obj([[0, 2, 0, 2, 0], [10, 0, 20, 20, 15]]))
        il = Dataset([Field([{'paris': [2.35, 48.87]}, [4.83, 45.76], [5.38, 43.3]],
                   name='location')])
        self.assertEqual(
    ...
    def test_creation_variable(self):
        ...
        il1 = Dataset.obj([['namvalue', ['a', 'b', 'c', 'd', 'e', 'f'], -1],
                     ['datationvalue', [10, 20, 30]],
                     ['locationvalue', [100, 200, 300], 1],
                    ['propertyvalue', [True, False]]])
        ...
        self.assertTrue(il1 == Dataset(il1.lindex, lvarname=il1.lvarname) == copy(il1))
    ...
    def test_creation_dic_ext(self):
        iidx = Dataset.dic({'datationvalue': [10, 10, 20, 20, 30, 30],
                           'locationvalue': [100, 100, 200, 200, 300, 300],
                           'propertyvalue': [True, False, True, False, True, False]})
        ...
        try:
            il3 = Dataset.dic({'i0': [1, 2, 3], 'i1': [[4, 5, 6], 0], 'i2': [
    ...
    def test_creation_dic_ext_variable(self):
        iidx = Dataset.dic({'varvalue': ['a', 'b', 'c', 'd', 'e', 'f'],
                           'datationvalue': [10, 10, 20, 20, 30, 30],
                           'locationvalue': [100, 100, 200, 200, 300, 300],
                           'propertyvalue': [True, False, True, False, True, False]},
                          var=0)
        ...
        iidx2 = Dataset.ext([[10, 10, 20, 20, 30, 30],
                           ...
                           var=3)
        iidx4 = Dataset.obj([['varvalue', ['a', 'b', 'c', 'd', 'e', 'f'], -1],
                       ['datationvalue', [10, 10, 20, 20, 30, 30]],
                       ['locationvalue', [100, 100, 200, 200, 300, 300]],
                       ['propertyvalue', [True, False, True, False, True, False]]])
    ...
    def test_properties(self):
        il = Dataset.obj([['ext', ['er', 'rt', 'er', 'ry'], -1], [0, 2, 0, 2],
                   [30, 12, 12, 15], [2, 0, 2, 0], [2, 2, 0, 0],
                   ['info', 'info', 'info', 'info'], [12, 12, 15, 30]])
        # il.reindex()
        ...
        il = Dataset.obj([['ext', ['er', 'rt', 'er', 'ry'], -1],
                   [0, 2, 0, 0], [30, 12, 20, 20]])
        # il.reindex()
    ...
    def test_addindex(self):
        iidx = Dataset.obj([['a', 'b', 'c'], [1, 2, 2], [4, 5, 5]])
    ...
    def test_add(self):
        ...
        il2 = Dataset.obj([[['_er', '_rt', '_er', '_ry', '_ab'], -1], [10, 2, 10, 12, 10],
                     [110, 0, 120, 120, 115]])
        il3 = il1 + il2
        self.assertEqual(len(il3), len(il1) + len(il2))
    ...
    def test_swap(self):
        il = Dataset.obj([[10, 20, 30], [[100, 200, 300], 0], [True, False]])
        il1 = copy(il)
        il.swapindex([2, 0, 1])
        il.swapindex([2, 0, 1])
        il.swapindex([2, 0, 1])
    ...
    def test_extend(self):
        il1 = Dataset.ext([['er', 'rt', 'er', 'ry', 'ab'], [
                    0, 2, 0, 2, 0], [10, 0, 20, 20, 15]])
        il2 = Dataset.obj([[['_er', '_rt', '_er', '_ry', '_ab'], -1], [10, 12, 10, 12, 10],
                     [110, 10, 120, 120, 115]])
        il3 = il1 | il2
        self.assertEqual(il3.lenidx, il2.lenidx)
        il = Dataset.ext([['er', 'rt', 'er', 'ry', 'ab', 'ert']], var=0)
        ilx = Dataset.ext([[0, 0, 0, 1, 1, 1], [0, 1, 2, 3, 4, 1]])
        il2 = il | ilx
    ...
    def test_append(self):
        il = Dataset.ext([[0, 2, 0, 2], [30, 12, 20, 15]])
        il.append([0, 20], unique=True)
        self.assertEqual(len(il), 4)
        il.append([0, 40])
        self.assertEqual(len(il), 5)
        self.assertEqual(il, Dataset.ext([[0, 2, 0, 2, 0], [30, 12, 20, 15, 40]]))
        il = Dataset.ext([[{'paris': [2.35, 48.87]}, [4.83, 45.76],
                   [5.38, 43.3]]], 'location')
    ...
    def test_append_variable(self):
        il = Dataset.obj([[['er', 'rt', 'er', 'ry'], -1],
                   [0, 2, 0, 2], [30, 12, 20, 15]])
        il.append(['truc', 0, 20], unique=True)
        self.assertEqual(len(il), 4)
        il.append(['truc', 0, 40])
        self.assertEqual(len(il), 5)
        self.assertEqual(il, Dataset.ext([['er', 'rt', 'er', 'ry', 'truc'],
                                    [0, 2, 0, 2, 0], [30, 12, 20, 15, 40]], var=0))

    def test_magic(self):
        iidx5 = Dataset.obj([['datationvalue', [10, 10, 20, 20, 30, 30]],
                       ['locationvalue', [100, 100, 200, 200, 300, 300]],
                       ['propertyvalue', [True, False, True, False, True, False]]])
        self.assertEqual(iidx5.lidx[0], Field.ext(
    ...
    def test_primary(self):
        #['ext', ['er', 'rt', 'er', 'ry']]
        ilm = Dataset.ext([['math', 'english', 'software', 'physic', 'english', 'software'],
    ...
    def test_to_obj(self):
        ilm = Dataset.ext([[0,     0,   2,   3,   4,   4,   6,   7,   8,   9,   9,  11,  12],
                          ['j', 'j', 'f', 'a', 'm', 'm', 's',
                              's', 's', 'n', 'd', 'd', 'd'],
        ...
        for ts in test:
            ...
            self.assertEqual(Dataset.from_obj(ilm.to_obj()), ilm)
        ...
        ilm = Dataset.ext([[6, 7, 8, 9, 9, 11, 12],
                          ['s', 's', 's', 'n', 'd', 'd', 'd']])
        ilm.coupling()
        for ts in test:
            opt = {'encoded': ts[0], 'format': ts[1], 'modecodec': ts[2]}
    ...
    def test_to_obj_variable(self):
        il = Dataset.ext([[0, 1, 2, 3, 4, 5],
                         ...
                         [100, 100, 200, 200, 300, 300],
                         [True, False, True, False, True, False]], var=0)
        self.assertEqual(Dataset.from_obj(il.to_obj()), il)
        il.setcanonorder()
        self.assertEqual(Dataset.from_obj(il.to_obj()), il)
        self.assertEqual(Dataset.from_obj(), Dataset())

    def test_coupling(self):
        ilx = Dataset.ext([['a', 'b', 'b', 'c', 'c', 'a'],
        ...
        il = Dataset.obj([[[1, 2, 3, 4, 5, 6], -1], ['a', 'b', 'b', 'c', 'c', 'a'],
                    [20,  10,  10,  10,  10,  20], [200, 200, 300, 200, 300, 300]])
    ...
    def test_full(self):
        ...
        il = Dataset.ext([['er', 'rt', 'er', 'ry'], [0, 2, 0, 2], [30, 12, 20, 30],
                         [2, 0, 2, 0], [2, 2, 0, 0], [
            'info', 'info', 'info', 'info'],
            [12, 20, 20, 12]], var=0)
        ilc = il.full()
    ...
    def test_merge(self):
        ...
        il3 = Dataset.ext([[il1, il2]], typevalue=None, var=0)
        self.assertEqual(il3.merge(mergeidx=True, updateidx=True).loc(
            ["english", "gr1"]), [14])
    ...
    def test_csv(self):
        il = Dataset.obj([[['er', 'rt', 'er', 'ry'], -1],
                   [0, 2, 0, 2], [30, 12, 20, 15]])
        il.to_csv('test.csv')
        il2 = Dataset.from_csv('test.csv', var=0)
    ...
    def test_filter(self):
        ...
        self.assertTrue(Dataset.ext([[], [], []]) == ilfr1 == ilfr2)
        '''
        il = Dataset.ext(f,l).setfilter([[0, 2], [12, 20, 30]], inplace=False, index=False)
    ...
    def test_to_xarray(self):
        ilm = Dataset.obj([['plants', ['fruit', 'fruit', 'fruit', 'fruit', 'vegetable', 'vegetable', 'vegetable', 'fruit']],
        ...
        '''il = Dataset.ext({'locatio': [0, [4.83, 45.76], [5.38, 43.3]],
    ...
    def test_to_obj_file(self):  # !!!
        il = Dataset.obj([['result', [0, 1, 2, 3, 4, 5], -1],
    ...
    '''for forma in ['json', 'cbor']:
        #for forma in ['json', 'cbor']:
            for encoded in [False, True]:
                for codif in [ES.codeb, {}]:
                    il2 = Dataset2.from_obj(il.to_obj(encoded=encoded, format=forma, codif=codif))
        ...
        il3 = Dataset2.from_obj(il.to_obj(encoded=False))
