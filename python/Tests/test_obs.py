...
def _res(n):
    return (ES.res_classES, _val(n))


def _dat(n):
    return (
        ES.dat_classES,
        [datetime.datetime(2000 + i, 2, 4, 12, 5, 0) for i in range(n)],
    )
...
pol13 = {
    dpt[12]["properties"]["code"] + " " + dpt[12]["properties"]["nom"]: dpt[12][
        "geometry"
    ]["coordinates"]
}
pol69 = {
    dpt[69]["properties"]["code"] + " " + dpt[69]["properties"]["nom"]: dpt[69][
        "geometry"
    ]["coordinates"]
}
pol75 = {
    dpt[75]["properties"]["code"] + " " + dpt[75]["properties"]["nom"]: dpt[75][
        "geometry"
    ]["coordinates"]
}
...
def _indic(res):
    return [
        res.json(encoded=True, format="json", json_res_index=True),
        res.dim,
        res.nMax,
        res.nInd,
        res.axes,
...
def dat(n):
    return [
        "datation",
        [datetime.datetime(2022, 9, i).isoformat() for i in range(1, n + 1)],
    ]
...
def slt(n):
    return [
        ...
        [
            [
                datetime.datetime(2022, 9, i).isoformat(),
                datetime.datetime(2022, 9, i + 1).isoformat(),
            ]
...
def res(n):
    return ["result", [i for i in range(n)], -1]


def stg(n):
    return ["string", ["example" + str(i) for i in range(n)]]


def dic(n):
    return ["dict", [{"example": "value" + str(i)} for i in range(n)]]


def lis(n):
    return ["list", [list(range(i + 1)) for i in range(1, n + 1)]]


def mix(n):
    return ["mixte", [[stg, dic, lis][i % 3](n)[1][i - 1] for i in range(1, n + 1)]]
...
def ppobs(listobs, param=None, name=None, info=False, mode="a"):
    ob = Observation.obj(
        {"name": name, "data": listobs, "param": param}
    ).setcanonorder()
    return printf(ob.json(encoded=True, json_info=info), ob, mode)
...
class TestExamples(unittest.TestCase):
    def test_obsjson(self):
        ...
        self.assertTrue(
            ppobs(
                [dat(3), loc(3), prp(2), p(stg, 3, 0), res(18)],
                param={"dimension": 3},
                name="example4",
                info=True,
            )
    ...
    def test_observation_for_sensor(self):
        ...
        for i in range(6):  # simule une boucle de mesure
            obs_sensor.append(
                [
                    45 + i,
                    datetime.datetime(2021, 6, 4 + i, 12, 5),
                    [1.1 + i, 40.2 + i],
                    prop1,
                ]
            )
        ...
        # data decoding in the server
        il_receive1 = Dataset.obj(payload1)
        il_receive2 = Dataset.obj(payload2)
        date_receive = datetime.datetime(2021, 6, 4, 12, 5)
        # print(ob_receive1 == ob_receive2 == ob_sensor)   # it's True !!
        self.assertTrue(il_receive1 == il_receive2 == il_operat)  # it's True !!
        # complete observation
        ob_complet = Observation.dic(
            {
                "res": il_receive1,
                "datation": date_receive,
                "location": [coord],
                "property": prop,
            }
        ).merge()
        ...
        # sensor : Dataset acquisition
        """il_sensor = Dataset.obj([['res', [], 0], ['datation', []],
                           ['property', [prop1, prop2], []]])"""
        il_sensor = Dataset.obj([["res", [], -1], ["datation", []], ["property", []]])
        property = [prop1, prop2]
        for i in range(6):  # simule une boucle de mesure
            date = datetime.datetime(2021, 6, 4 + i, 12, 5)
        ...
        # data decoding in the server
        il_receive = Dataset.obj(payload, reindex=False)
        self.assertTrue(il_receive == il_sensor)  # it's True !!
        il_receive.nindex("property").setcodeclist(property)
        # complete observation
        ob_complet = (
            Observation.dic({"res": il_receive, "location": [coord]})
            .merge()
            .setcanonorder()
        )
        # print(ob_complet.to_obj(encoded=True))
        self.assertTrue(
            ob_complet.dimension == 2 and ob_complet.complete
        )  # it's True !!
...
class TestObservation(unittest.TestCase):
    ...
    def test_obs_creation_copy(self):
        listob = [
            ...
            Observation(
                Dataset.obj([list(dat1), list(loc1), list(prop3), list(_res(3))]),
                name="truc",
            ),
            Observation.obj(
                {
        ...
        ob3 = Observation.std(
            datation=dat1[1], location=loc1[1], property=prop3[1], result=_res(3)[1]
        )
        ob6 = Observation.std(
            datation=dat1[1][0],
            location=loc1[1][0],
    ...
    def test_obs_loc_iloc_maj(self):
        ob = Observation(
            Dataset.obj(
                [list(dat3), list(loc3) + [0], list(prop2), list(_res(6)) + [-1]]
            ),
            param=truc_mach,
        )
        self.assertTrue(
            ...
            == ob.loc(
                [
                    datetime.datetime.fromisoformat(dat3[1][1]),
                    loc3[1][1],
                    prop2[1][1],
                    3,
                ],
            ...
            == ob.loc(
                [
                    DatationValue(dat3[1][1]),
                    LocationValue(loc3[1][1]),
        ...
        self.assertTrue(
            ob.nindex("result").loc(5)
            == ob.nindex("result").loc(NamedValue(5), extern=False)
            == [5]
        )
        self.assertTrue(
            ob.nindex("datation").loc("2021-07-04T10:05:00")
            == ob.nindex("datation").loc(
                DatationValue("2021-07-04T10:05:00"), extern=False
            )
            == [2, 3]
        )
    ...
    def test_obs_vList(self):
        ...
        ob = Observation(Dataset.obj([list(dat3), list(dpt2)]), param=truc_mach)
    ...
    def test_obs_add(self):
        ob = Observation.obj(
            {"data": [list(dat3), list(loc3), list(prop2), list(_res(18)) + [-1]]}
        )
        obp = Observation.obj(
            {"data": [list(pdat3), list(ploc3), list(pprop2), list(_res(18)) + [-1]]}
        )
        obc = obp + ob
        self.assertEqual(
            set(obc.lindex[0].codec), set(obp.lindex[0].codec + ob.lindex[0].codec)
        )
        self.assertEqual(
            obc.lindex[3].values, obp.lindex[3].values + ob.lindex[3].values
        )
        ob2 = ob + ob
    ...
    def test_obs_full(self):
        ...
        ob.full(
            fillvalue=-1, idxname=["datation", "location", "property"], inplace=True
        )
    ...
    def test_json_file(self):
        res = (
            "result",
            [{"file": "truc", "path": "ertert"}, 1, 2, 3, 4, ["truc", "rt"]],
        )
        ob = Observation.obj(
...
class TestExports(unittest.TestCase):
    ...
    def test_geo_interface(self):
        ...
        self.assertEqual(
            set(ob.__geo_interface__["coordinates"][0]), set(dpt2pt["coordinates"][0])
        )
...
class TestInterne(unittest.TestCase):
    ...
    @unittest.skipIf(mongo, "test envoi mongo")
    def test_plot(self):
        il3 = Dataset.dic({'location': [[0,1], [4.83, 45.76], [5.38, 43.3]],
        ...
        il2 = Dataset.dic({'locatio': [0, [4.83, 45.76], [5.38, 43.3]],
        ...
        il2 = Dataset.dic({'location': [[1,2], [4.83, 45.76], [5.38, 43.3]],
         'datation': [[{'date1': '2021-02-04T11:05:00+00:00'},
           '2021-07-04T10:05:00+00:00',
           '2021-05-04T10:05:00+00:00'],
          0],
        ...
        il1 = Dataset.dic({'locatio': [0, [4.83, 45.76], [5.38, 43.3]],
         'datatio': [[{'date1': '2021-02-04T11:05:00+00:00'},
           '2021-07-04T10:05:00+00:00',
           '2021-05-04T10:05:00+00:00'],
          0],
        ...
        il1 = Dataset.dic({'location': [[1,2], [4.83, 45.76], [5.38, 43.3]],
         'datation': [[{'date1': '2021-02-04T11:05:00+00:00'},
           '2021-07-04T10:05:00+00:00',
           '2021-05-04T10:05:00+00:00'],
          0],
