...
def to_json(ndarray, axes=None):
    ...
    if axes:
        ...
        return {"data": [data, dtype], "xnames": axes_name, "xvalues": axes_values}
    return [data, dtype]
...
def from_json(js):
    if isinstance(js, dict):
        return (
            np.array(js["data"][0], dtype=js["data"][1]),
...
def to_json_tab(ndarray, axes=None):
    ...
    axes_val = (
        ...
        else [list(range(period[i])) for i in range(dim)]
    )
...
def from_json_tab(js):
    ...
    for name, value in js.items():
        if len(value) == 2 and isinstance(value[1], list) and len(value[1]) == 1:
            ...
            spl = name.split("::")
            ndarray = (
                np.array(value, dtype=spl[1]) if len(spl) == 2 else np.array(value)
            )
