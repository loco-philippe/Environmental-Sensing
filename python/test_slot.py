...
class TestObsUnitaire(unittest.TestCase):
    ...
    def test_link(self):
        ...
        self.assertEqual(ts1.link(ts1), 'full equal')
        self.assertEqual(ts1.link(ts2), 'full disjoint')
        self.assertEqual(ts2.link(ts1), 'full disjoint')
        ts2 = TimeSlot([_inte(11,14), _inte(8,10)])
        self.assertEqual(ts1.link(ts2), 'disjoint')
        self.assertEqual(ts2.link(ts1), 'disjoint')
        ts2 = TimeSlot([_inte(2,3), _inte(6,9)])
        self.assertEqual(ts1.link(ts2), 'stacked')
        self.assertEqual(ts2.link(ts1), 'stacked')
        ts1 = TimeSlot([_inte(1,4), _inte(5,10)])
        self.assertEqual(ts1.link(ts2), 'full superset')
        self.assertEqual(ts2.link(ts1), 'full subset')
